...
**[百人一首クイズを試す](https://kazunori-kimura.github.io/100nin-isssyu/)**
...
### 3. 設定機能
- **出題方法選択**: 範囲選択 or 個別選択の2つのモード
- **範囲選択**: 1-100番の連続した範囲を指定（例: 1-50番）
- **個別選択**: 好きな歌を自由に選択（検索・フィルタ機能付き）
- **バリデーション**: 適切な設定の検証機能（最低4首必要）
...
### 設定画面（`/settings`）
- **モード選択**: 範囲選択/個別選択の切り替え
- **範囲選択モード**: 開始・終了番号入力フィールド（1-100）
- **個別選択モード**: 
  - 全100首の歌一覧表示
  - 歌番号・歌詞・歌人名での検索機能
  - クリックで選択/解除の直感的操作
...
# デプロイ準備（.nojekyllファイル作成込み）
npm run deploy
```

### GitHub Pages デプロイメント
このアプリは自動的にGitHub Pagesにデプロイされます：

1. **自動デプロイ**: mainブランチへのpush時に自動実行
2. **GitHub Actions**: `.github/workflows/deploy.yml`で定義
3. **静的サイト**: Next.js Export機能で完全に静的化
4. **カスタムドメイン対応**: 必要に応じて設定可能

**ライブURL**: https://kazunori-kimura.github.io/100nin-isssyu/
